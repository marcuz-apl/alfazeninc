...
export async function POST(request: NextRequest) {
  ...
  try {
    const { title, content, show_contact_us } = await request.json();
    if (title === undefined || content === undefined || show_contact_us === undefined) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
    }

    db.prepare("INSERT OR REPLACE INTO hero_settings (id, title, content, show_contact_us) VALUES (1, ?, ?, ?)")
      .run(title, content, show_contact_us ? 1 : 0);
